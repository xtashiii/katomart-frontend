...
export default function BackupsPage() {
  ...
  const handleCreateBackup = async () => {
    try {
      const selectedMaterialName = downloadedMaterials.find(m => m.id === selectedMaterial)?.name || '';
      const selectedCloudName = rcloneClouds.find(c => c.id === selectedCloud)?.profile + ' (' + rcloneClouds.find(c => c.id === selectedCloud)?.service + ')' || '';
      ...
      const response = await fetch('/api/admin/backups', {
        ...
        body: JSON.stringify({
          ...
          type: backupType,
          zipType: zipType,
          zipSize: zipType === 'separate' || zipType === 'dependent' ? zipPresets.find(p => p.value === zipPreset)?.label || '-' : '-',
      ...
      <div style={{ display: 'flex', gap: 24 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 10 }}>
            <label><input type="radio" name="backupType" value="mirror" checked={backupType === 'mirror'} onChange={() => setBackupType('mirror')} /> {t('mirror')}</label>
            <label><input type="radio" name="backupType" value="move" checked={backupType === 'move'} onChange={() => setBackupType('move')} /> {t('move')}</label>
            <label style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <input type="checkbox" checked={keepFilesOnDisk} onChange={(e) => setKeepFilesOnDisk(e.target.checked)} />
              ...
              {keepFilesOnDisk && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 10 }}>
                  ...
                  />
                  <button className="app-button" type="button" style={{ padding: '6px 12px' }}>{t('browse')}</button>
                </div>
              )}
            </label>
          </div>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <input type="radio" name="zipType" value="separate" checked={zipType === 'separate'} onChange={() => setZipType('separate')} />
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <input type="radio" name="zipType" value="dependent" checked={zipType === 'dependent'} onChange={() => setZipType('dependent')} />
      ...
      <div style={{ display: 'flex', gap: 24 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ marginTop: 8, display: 'flex', gap: 16 }}>
            <label><input type="checkbox" checked={videoCompression === 'x265'} onChange={(e) => setVideoCompression(e.target.checked ? 'x265' : 'none')} /> {t('compressToX265')}</label>
          </div>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginTop: 8 }}>
        <button className="app-button" style={{ background: '#2196f3', color: 'white' }} onClick={handleCreateBackup}>{t('startBackup')}</button>
        <button className="app-button" style={{ background: '#4caf50', color: 'white' }} onClick={handleTestBackup}>{t('testBackup')}</button>
      </div>
    </form>
  );
        ...
        {backups.map(b => (
          <div key={b.id} style={{
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 8 }}>
              ...
              <button className="app-button" style={{ background: '#f44336', color: 'white' }} onClick={() => handleDeleteBackup(b.id)}>{t('delete')}</button>
            </div>
          </div>
        ))}
      </div>
    ...
    <div>
      ...
      <main className="main-content">
        <div className="content-wrapper" style={{ maxWidth: 1100, margin: '0 auto', padding: '2.5rem 2rem' }}>
          <h1 className="brand-title" style={{ marginBottom: 32 }}>{t('backup')}</h1>
