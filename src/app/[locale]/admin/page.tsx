...
export default function AdminPage() {
  ...
  const [pendingDeleteRclone, setPendingDeleteRclone] = useState<string | null>(null);
  ...
  const [mediaServerInstanceEnabled, setMediaServerInstanceEnabled] = useState(false);
  ...
  const [downloadConfigSubTab, setDownloadConfigSubTab] = useState('file-system');
  const [downloadConfigTabs, setDownloadConfigTabs] = useState<DownloadConfigTab[]>([]);
  ...
  useEffect(() => {
    ...
    const loadAdminData = async () => {
      try {
        ...
        setDownloadConfigTabs(configTabsData.tabs?.map((tab: { id: string; label: string }) => ({
          id: tab.id,
          label:
            tab.id === 'file-system' ? t('fileSystem') :
            tab.id === 'yt-dlp' ? t('ytDlp') :
            tab.id === 'media-types' ? t('mediaTypes') :
            tab.id === 'media-decryption' ? t('mediaDecryption') :
            tab.id === 'video-postprocessing' ? t('videoPostprocessing') :
            t(tab.label)
        })) || []);
    ...
    if (pendingDeleteRclone) {
      try {
        const response = await fetch('/api/admin/rclone-clouds', {
          ...
          body: JSON.stringify({ action: 'delete', cloud: { id: pendingDeleteRclone } })
        });
        const data = await response.json();
        if (data.success) {
          setRcloneClouds(prev => prev.filter(c => c.id !== pendingDeleteRclone));
        }
      } catch (error) {
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 16 }}>
              <thead>
                <tr style={{ background: 'var(--background)' }}>
                  ...
                  <th style={{ textAlign: 'center', padding: '10px 8px', fontWeight: 600 }}>{t('delete')}</th>
                </tr>
              </thead>
              <tbody>
                {rcloneClouds.map(cloud => (
                  ...
                  <tr style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '10px 8px', textAlign: 'center' }}>
                      <button
                        className="app-button"
                        style={{ background: '#dc3545', color: 'white', padding: '4px 10px', fontSize: 15, borderRadius: 6, border: 'none', cursor: 'pointer' }}
                        title={t('removeRcloneCloud', { default: 'Remove cloud' })}
                        onClick={() => handleDeleteRclone(cloud.id, cloud.profile)}
                    ...
                    <tr>
                      <td colSpan={7} style={{ padding: 0 }}>
                        <div style={{
                          ...
                          <span>
                            {t('warningDeleteCloud', { profile: pendingDeleteProfile || '' })}
                          </span>
                          <span>
                            <button className="app-button" style={{ background: '#dc3545', color: 'white', marginRight: 12 }} onClick={confirmDeleteRclone}>
                              {t('yesRemove')}
                            </button>
                            <button className="app-button" style={{ background: '#eee', color: '#333' }} onClick={cancelDeleteRclone}>
                              {t('cancel')}
                            </button>
                          </span>
                        </div>
  ...
  const AuthConfigTab = () => (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      <div className="flex border-b border-gray-200 mb-4" style={{ gap: 0, justifyContent: 'space-between' }}>
        <button
  ...
  // const BackupListTab = () => (
    <div style={{ marginTop: 16, maxWidth: 900, marginLeft: 'auto', marginRight: 'auto' }}>
      {backups.length === 0 && <div style={{ color: '#888', textAlign: 'center' }}>{t('noBackupsFound')}</div>}
        ...
        {backups.map(b => (
          <div key={b.id} style={{
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 24, alignItems: 'center', marginBottom: 8 }}>
              <div style={{ flex: 2, minWidth: 180 }}><b>{t('material')}:</b> {b.material}</div>
              <div style={{ flex: 2, minWidth: 180 }}><b>{t('cloud')}:</b> {b.cloud}</div>
              <div style={{ flex: 1, minWidth: 120 }}><b>{t('type')}:</b> {b.type}</div>
              <div style={{ flex: 1, minWidth: 120 }}><b>{t('zip')}:</b> {b.zip}</div>
              <div style={{ flex: 1, minWidth: 120 }}><b>{t('splitSize')}:</b> {b.zipSize}</div>
              <div style={{ flex: 1, minWidth: 120 }}><b>{t('date')}:</b> {b.date}</div>
            </div>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 8 }}>
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="app-button" style={{ background: '#2196f3', color: 'white' }}>{t('retrieveMirror')}</button>
                <button className="app-button" style={{ background: '#2196f3', color: 'white' }}>{t('retrieveMove')}</button>
              </div>
              <button className="app-button" style={{ background: '#4caf50', color: 'white' }}>{t('test')}</button>
              <button className="app-button" style={{ background: '#f44336', color: 'white' }}>{t('delete')}</button>
            </div>
          </div>
        ))}
      </div>
  ...
  */
  ...
  const handleAddCategory = async () => {
    ...
    try {
      const response = await fetch('/api/admin/categories', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'add', category: { name: newCategory.trim() } })
      });
      const data = await response.json();
      if (data.success) {
  ...
  const CognitahzConfigTab = () => (
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
      ...
      <div className="flex border-b border-gray-200 mb-4" style={{ gap: 0, justifyContent: 'space-between' }}>
        ...
        >{t('localPlatform')}</button>
      </div>
      <div>
        {cognitahzSubTab === 'media-server' && (
          <form style={{ display: 'flex', flexDirection: 'column', gap: 32 }} onSubmit={e => e.preventDefault()}>
            {/* Media Server Connection Settings */}
            <div style={{ background: '#f8fafc', padding: '24px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
              <h4 style={{ marginBottom: 20, fontSize: 18, fontWeight: 600, color: '#1a237e' }}>{t('connectionSettings')}</h4>
            ...
            <div style={{ background: '#f0f9ff', padding: '24px', borderRadius: '12px', border: '1px solid #bae6fd' }}>
              <h4 style={{ marginBottom: 20, fontSize: 18, fontWeight: 600, color: '#1a237e' }}>{t('mediaServerInstance')}</h4>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 12, fontWeight: 500, fontSize: 16 }}>
                  <input 
                    type="checkbox" 
                    checked={mediaServerInstanceEnabled}
                    onChange={e => setMediaServerInstanceEnabled(e.target.checked)}
                ...
                {mediaServerInstanceEnabled && (
                  <div style={{ marginLeft: 30, display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    {requireLogin && (
                      <label style={{ display: 'flex', alignItems: 'center', gap: 12, fontWeight: 500, marginLeft: 28 }}>
                        <input 
                          type="checkbox" 
                          checked={requireUsernameMatch}
                          onChange={e => setRequireUsernameMatch(e.target.checked)}
                          style={{ width: 16, height: 16, cursor: 'pointer' }}
                        />
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 8, gap: 16 }}>
              ...
              <button className="app-button" style={{ background: '#1976d2', color: 'white', minWidth: 180 }}>{t('saveMediaServerConfig')}</button>
            </div>
          </form>
        )}
        {cognitahzSubTab === 'local-platform' && (
          <div style={{
            ...
            gap: 32,
            alignItems: 'start',
            background: 'linear-gradient(135deg, #f8fafc 0%, #f3f6fb 50%, #eef2f7 100%)',
            borderRadius: 16,
            ...
            <div style={{ 
              ...
              <h4 style={{ marginBottom: 18, fontSize: 21, fontWeight: 700, color: '#1a237e', letterSpacing: 0.2 }}>{t('categories')}</h4>
              <div style={{ display: 'flex', gap: 10, marginBottom: 22 }}>
                <input
                  ...
                  style={{ flex: 1, fontSize: 16, padding: '10px 14px', background: '#f5f7fa', color: '#222', border: '1px solid #e0e4ea', borderRadius: 7 }}
                  onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); handleAddCategory(); } }}
                />
                <button className="app-button" style={{ background: 'linear-gradient(90deg,#1976d2 60%,#2196f3 100%)', color: 'white', fontWeight: 600, fontSize: 16, padding: '10px 22px', borderRadius: 7, boxShadow: '0 2px 8px 0 rgba(25,118,210,0.08)' }} onClick={handleAddCategory}>{t('add')}</button>
              </div>
              <div style={{ 
                maxHeight: 350, 
                overflowY: 'auto', 
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                  {categories.length === 0 && <li style={{ color: '#888', fontSize: 16, padding: '20px 12px', textAlign: 'center', fontStyle: 'italic' }}>{t('noCategoriesYet')}</li>}
                  {categories.map(cat => (
                    <li key={cat.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px', borderBottom: '1px solid #e2e8f0' }}>
                      <span style={{ fontSize: 16, color: '#222', fontWeight: 500, flex: 1 }}>{cat.name}</span>
                      <button className="app-button" style={{ background: 'linear-gradient(90deg,#f44336 60%,#e57373 100%)', color: 'white', fontSize: 14, padding: '6px 12px', borderRadius: 6, fontWeight: 500, boxShadow: '0 2px 6px 0 rgba(244,67,54,0.15)', minWidth: 80, textAlign: 'center' }} onClick={() => handleDeleteCategory(cat.id)}>{t('delete')}</button>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div style={{ 
              ...
              <h4 style={{ marginBottom: 18, fontSize: 21, fontWeight: 700, color: '#1a237e', letterSpacing: 0.2 }}>{t('courseNotes')}</h4>
              <div style={{ marginBottom: 20, color: '#4b5563', fontSize: 16 }}>{t('courseNotesDescription')}</div>
              <div style={{ 
                maxHeight: 350, 
                overflowY: 'auto', 
                border: '1px solid #e2e8f0', 
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                  {courseNotes.map(course => (
                    <li key={course.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 12px', borderBottom: '1px solid #e2e8f0' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, marginRight: 12 }}>
                        <span style={{ fontSize: 16, color: '#222', fontWeight: 500 }}>{course.name}</span>
                        <span style={{ color: course.notes > 0 ? '#1976d2' : '#aaa', fontWeight: 600, fontSize: 14, marginTop: 4 }}>{course.notes} {t('notes')}</span>
                      </div>
                      <button className="app-button" style={{ background: course.notes > 0 ? 'linear-gradient(90deg,#1976d2 60%,#2196f3 100%)' : '#e2e8f0', color: course.notes > 0 ? 'white' : '#888', fontSize: 14, padding: '8px 16px', borderRadius: 6, fontWeight: 500, boxShadow: course.notes > 0 ? '0 2px 6px 0 rgba(25,118,210,0.15)' : 'none', cursor: course.notes > 0 ? 'pointer' : 'not-allowed', minWidth: 120, textAlign: 'center' }} disabled={course.notes === 0} title={course.notes === 0 ? t('noNotesToDownload') : t('downloadPdfTooltip')}>
                        {t('downloadPdf')}
                      </button>
                    </li>
                  ))}
  ...
  const DownloadConfigTab = () => (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 2px 12px 0 rgba(60,80,120,0.06)', padding: '2.5rem 2rem', minHeight: 220, marginTop: 8 }}>
        {downloadConfigSubTab === 'file-system' && (
          <form style={{ maxWidth: 700, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 28 }} onSubmit={e => e.preventDefault()}>
            <div style={{ marginBottom: 10, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <label style={{ fontWeight: 700, fontSize: 18, display: 'block', marginBottom: 6 }}>{t('maxPathLength')}</label>
              <input type="number" className="app-button" style={{ width: 180, background: '#fff', color: '#222', fontSize: 15, padding: '8px 12px', marginBottom: 8 }} min="50" max="300" placeholder="260" />
              <span style={{ color: '#b71c1c', fontSize: 14, textAlign: 'center' }}>
                {t('windowsPathWarning')}
              </span>
            </div>
            <div style={{ display: 'flex', gap: 24 }}>
              <div style={{ flex: 1 }}>
                <label style={{ fontWeight: 600 }}>{t('maxCourseNameLength')}</label>
                <input type="number" className="app-button" style={{ width: '100%', background: '#fff', color: '#222', marginTop: 8 }} min="10" max="100" placeholder="50" />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ fontWeight: 600 }}>{t('maxModuleNameLength')}</label>
                <input type="number" className="app-button" style={{ width: '100%', background: '#fff', color: '#222', marginTop: 8 }} min="10" max="100" placeholder="50" />
              </div>
            </div>
            <div style={{ display: 'flex', gap: 24 }}>
              <div style={{ flex: 1 }}>
                <label style={{ fontWeight: 600 }}>{t('maxLessonNameLength')}</label>
                <input type="number" className="app-button" style={{ width: '100%', background: '#fff', color: '#222', marginTop: 8 }} min="10" max="100" placeholder="50" />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ fontWeight: 600 }}>{t('maxFileNameLength')}</label>
                <input type="number" className="app-button" style={{ width: '100%', background: '#fff', color: '#222', marginTop: 8 }} min="10" max="100" placeholder="50" />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 18 }}>
              <button className="app-button" style={{ background: '#fff', color: '#1976d2', fontWeight: 600, fontSize: 16, padding: '10px 32px', borderRadius: 7, border: '1px solid #1976d2' }}>{t('saveFileSystemConfig')}</button>
            </div>
          </form>
        )}
        {downloadConfigSubTab === 'yt-dlp' && (
          <form style={{ maxWidth: 600, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 24 }} onSubmit={e => e.preventDefault()}>
            <div>
              <label style={{ fontWeight: 500 }}>{t('outputTemplate')}</label>
              <input type="text" className="app-button" style={{ width: '100%', marginTop: 8, background: '#fff', color: '#222' }} placeholder="e.g. %(title)s.%(ext)s" />
            </div>
            <div>
              <label style={{ fontWeight: 500 }}>{t('format')}</label>
              <input type="text" className="app-button" style={{ width: '100%', marginTop: 8, background: '#fff', color: '#222' }} placeholder="e.g. bestvideo+bestaudio/best" />
            </div>
            <div>
              <label style={{ fontWeight: 500 }}>{t('proxy')}</label>
              <input type="text" className="app-button" style={{ width: '100%', marginTop: 8, background: '#fff', color: '#222' }} placeholder="e.g. socks5://127.0.0.1:1080" />
            </div>
            <div>
              <label style={{ fontWeight: 500 }}>{t('cookiesFile')}</label>
              <input type="file" className="app-button" style={{ width: '100%', marginTop: 8, background: '#fff', color: '#222' }} />
            </div>
            <div style={{ display: 'flex', gap: 16 }}>
              <div style={{ flex: 1 }}>
                <label style={{ fontWeight: 500 }}>{t('retries')}</label>
                <input type="number" className="app-button" style={{ width: '100%', marginTop: 8, background: '#fff', color: '#222' }} min="1" max="20" placeholder="10" />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ fontWeight: 500 }}>{t('sleepInterval')}</label>
                <input type="number" className="app-button" style={{ width: '100%', marginTop: 8, background: '#fff', color: '#222' }} min="0" max="60" placeholder="0" />
              </div>
            </div>
            <div style={{ display: 'flex', gap: 16 }}>
              <div style={{ flex: 1 }}>
                <label style={{ fontWeight: 500 }}>{t('threads')}</label>
                <input type="number" className="app-button" style={{ width: '100%', marginTop: 8, background: '#fff', color: '#222' }} min="1" max="32" placeholder="1" />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ fontWeight: 500 }}>{t('fragmentRetries')}</label>
                <input type="number" className="app-button" style={{ width: '100%', marginTop: 8, background: '#fff', color: '#222' }} min="1" max="20" placeholder="10" />
              </div>
            </div>
            <div style={{ display: 'flex', gap: 16 }}>
              <div style={{ flex: 1 }}>
                ...
                <select className="app-button" style={{ width: '100%', marginTop: 8, background: '#fff', color: '#222' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 16 }}>
              <button className="app-button" style={{ background: '#fff', color: '#1976d2', fontWeight: 600, fontSize: 16, padding: '10px 32px', borderRadius: 7, border: '1px solid #1976d2' }}>{t('saveYtDlpConfig')}</button>
            </div>
          </form>
        )}
        {downloadConfigSubTab === 'media-types' && (
          <form style={{ maxWidth: 600, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 28 }} onSubmit={e => e.preventDefault()}>
            <div style={{ marginBottom: 8 }}>
              <label style={{ fontWeight: 600, fontSize: 17, display: 'block', textAlign: 'center', marginBottom: 10 }}>{t('convertMarkdownHtml')}</label>
              <div style={{ display: 'flex', gap: 32, justifyContent: 'center', marginTop: 10 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 500, fontSize: 16 }}>
                  <input type="checkbox" style={{ width: 18, height: 18 }} />
                  {t('markdownToPdf')}
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 500, fontSize: 16 }}>
            ...
            <div style={{ marginBottom: 8 }}>
              <label style={{ fontWeight: 600, fontSize: 17 }}>{t('allowedMediaExtensions')}</label>
              <input type="text" className="app-button" style={{ width: '100%', marginTop: 8, background: '#fff', color: '#222' }} placeholder="e.g. mp4, mp3, pdf, jpg, png, webm, mkv" />
              <div style={{ color: '#888', fontSize: 14, marginTop: 4 }}>{t('allowedExtensionsHelp')}</div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 8 }}>
              <input type="checkbox" id="allowNonMatching" style={{ width: 18, height: 18 }} />
              <label htmlFor="allowNonMatching" style={{ fontWeight: 500, fontSize: 16, cursor: 'pointer' }}>
                {t('allowNonMatchingExtensions')}
              </label>
            </div>
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 18 }}>
              <button className="app-button" style={{ background: '#fff', color: '#1976d2', fontWeight: 600, fontSize: 16, padding: '10px 32px', borderRadius: 7, border: '1px solid #1976d2' }}>{t('saveMediaTypesConfig')}</button>
            </div>
          </form>
        )}
        {downloadConfigSubTab === 'media-decryption' && (
          <form style={{ maxWidth: 600, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 32 }} onSubmit={e => e.preventDefault()}>
            <div>
              <label style={{ fontWeight: 600, fontSize: 17, display: 'block', marginBottom: 12 }}>{t('selectDrmSources')}</label>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                {/* Widevine */}
                <div style={{ background: '#f8fafc', border: '1px solid #e0e4ea', borderRadius: 8, padding: '1.2rem 1.5rem' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 10, fontWeight: 500, fontSize: 16, marginBottom: 10 }}>
                    <input type="checkbox" style={{ width: 18, height: 18 }} />
                    {t('widevine')}
                  </label>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 10 }}>
                    <label style={{ fontWeight: 500, fontSize: 15, marginBottom: 8 }}>{t('selectCdmFolder')}</label>
                    <label style={{ display: 'inline-block' }}>
                      <input
                        ...
                        directory="true"
                      />
                      <span className="app-button" style={{ background: '#fff', color: '#1976d2', border: '1px solid #1976d2', fontWeight: 600, fontSize: 15, padding: '8px 22px', borderRadius: 7, cursor: 'pointer', display: 'inline-block' }}>
                        {t('chooseFolder')}
                      </span>
                    </label>
                  </div>
                ...
                <div style={{ background: '#f8fafc', border: '1px solid #e0e4ea', borderRadius: 8, padding: '1.2rem 1.5rem' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 10, fontWeight: 500, fontSize: 16, marginBottom: 10 }}>
                    <input type="checkbox" style={{ width: 18, height: 18 }} disabled />
                    {t('fairplay')}
                  </label>
                  <div style={{ marginLeft: 28, color: '#888', fontSize: 15, marginTop: 6 }}>
                    {t('comingSoon')}
                  </div>
                </div>
                {/* Playready */}
                <div style={{ background: '#f8fafc', border: '1px solid #e0e4ea', borderRadius: 8, padding: '1.2rem 1.5rem' }}>
                  ...
                  <div style={{ marginLeft: 28, marginTop: 6, display: 'flex', alignItems: 'center', gap: 10 }}>
                    <input type="checkbox" id="playready-cve" style={{ width: 18, height: 18 }} />
                    <label htmlFor="playready-cve" style={{ fontWeight: 500, fontSize: 15, cursor: 'pointer' }}>
                      {t('useLatestCve')}
                    </label>
                  </div>
                </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 18 }}>
              <button className="app-button" style={{ background: '#fff', color: '#1976d2', fontWeight: 600, fontSize: 16, padding: '10px 32px', borderRadius: 7, border: '1px solid #1976d2' }}>{t('saveMediaDecryptionConfig')}</button>
            </div>
          </form>
        )}
        {downloadConfigSubTab === 'video-postprocessing' && (
          <form style={{ maxWidth: 600, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 28 }} onSubmit={e => e.preventDefault()}>
            <div style={{ fontWeight: 600, fontSize: 17, marginBottom: 10 }}>{t('videoPostprocessingOptions')}</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 10, fontWeight: 500, fontSize: 16 }}>
                <input type="checkbox" style={{ width: 18, height: 18 }} />
            ...
            <div style={{ marginTop: 18 }}>
              <label style={{ fontWeight: 600, fontSize: 16, marginBottom: 8, display: 'block' }}>{t('customFfmpegParams')}</label>
              <input type="text" className="app-button" style={{ width: '100%', background: '#fff', color: '#222', fontSize: 15, padding: '10px 14px' }} placeholder="e.g. -vf scale=1280:720 -b:v 1M -preset fast" />
              <div style={{ color: '#888', fontSize: 14, marginTop: 4 }}>{t('ffmpegParamsHelp')}</div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 18 }}>
              <button className="app-button" style={{ background: '#fff', color: '#1976d2', fontWeight: 600, fontSize: 16, padding: '10px 32px', borderRadius: 7, border: '1px solid #1976d2' }}>{t('saveVideoPostprocessingConfig')}</button>
            </div>
          </form>
        )}
      </div>
        ...
        <div className="content-wrapper" style={{ maxWidth: 1100, margin: '0 auto', padding: '2.5rem 2rem' }}>
          ...
          <div className="section-card" style={{ padding: '2.5rem 2rem', marginBottom: 40, width: '80vw', maxWidth: '1100px', marginLeft: 'auto', marginRight: 'auto' }}>
            ...
            <div style={{ marginBottom: 32 }}>
              <div className="flex border-b border-gray-300" style={{ fontSize: '1.15rem', justifyContent: 'center' }}>
                {tabs.map(tab => (
                  <button
                    key={tab.id}
