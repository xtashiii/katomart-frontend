...
export default function ScrappersPage() {
  ...
  const [loggedInLibraryPlatform, setLoggedInLibraryPlatform] = useState<string>('');
  const [downloadingBooks, setDownloadingBooks] = useState<Set<string>>(new Set());
  const [downloadNotifications, setDownloadNotifications] = useState<Array<{id: string, message: string, type: 'success' | 'info'}>>([]);
  ...
  const [selectedLibraryPlatform, setSelectedLibraryPlatform] = useState<string>('');
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const platformParam = loggedInLibraryPlatform ? `&platform=${encodeURIComponent(loggedInLibraryPlatform)}` : '';
        const response = await fetch(`/api/books?search=${encodeURIComponent(bookSearchQuery)}${platformParam}`);
        if (response.ok) {
          const data = await response.json();
          setBooks(data.books || []);
        }
  ...
  // Download workflow
  const handleStartDownload = async (config: { downloadPath: string; createSubfolders: boolean }) => {
    ...
    try {
      ...
      setDownloadState(ds => ({ ...ds, running: true, finished: false, paused: false, progress: {} }));
      ...
      content: (
        <div className="space-y-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">{tDownload('basicSettings')}</h3>
            <DownloadSettings onStart={handleStartDownload} config={downloadConfig} setConfig={setDownloadConfig} disabled={downloadState.running} />
          </div>

          {/* File Management Settings */}
          <div className="bg-gray-50 rounded-lg p-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">{t('fileManagement')}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t('fileNamingPattern')}</label>
                <input
                  type="text"
                  value={filePattern}
                ...
                />
                <p className="text-xs text-gray-500 mt-1">{t('filePatternHelp')}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t('allowedFileTypes')}</label>
                <input
                  type="text"
                  value={allowedTypes}
                  ...
                  disabled={downloadState.running}
                />
                <p className="text-xs text-gray-500 mt-1">{t('allowedTypesHelp')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">{t('performanceSettings')}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t('maxConcurrentDownloads')}</label>
                <input
                  type="number"
                  min={1}
                  max={10}
                  ...
                  disabled={downloadState.running}
                />
                <p className="text-xs text-gray-500 mt-1">{t('concurrentHelp')}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t('ffmpegArgs')}</label>
                <input
                  type="text"
                  value={ffmpegArgs}
                  ...
                  disabled={downloadState.running}
                />
                <p className="text-xs text-gray-500 mt-1">{t('ffmpegArgsHelp')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">{t('authSettings')}</h3>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{t('cookieFile')}</label>
              <input
                type="file"
                accept=".txt,.cookies"
                ...
                disabled={downloadState.running}
              />
              {cookieFile && <div className="text-xs text-gray-500 mt-1">{t('selected')} {cookieFile.name}</div>}
              <p className="text-xs text-gray-500 mt-1">{t('cookieFileHelp')}</p>
            </div>
          </div>
        </div>
      ),
  ...
  const showNotification = (message: string, type: 'success' | 'info' = 'info') => {
    ...
    setTimeout(() => {
      setDownloadNotifications(prev => prev.filter(notif => notif.id !== id));
    }, 4000);
  };
  ...
  const handleBookDownload = async (book: Book) => {
    ...
    setTimeout(() => {
      ...
      showNotification(t('downloadCompleted', { title: book.title }), 'success');
    }, 3000);
  };

  const LibraryLoginContent = () => {
    const [libLoginMethod, setLibLoginMethod] = useState<'credentials' | 'cookies'>('credentials');
    ...
    const selectedLibPlatform = libraryPlatforms.find(p => p.id === selectedLibraryPlatform);

    return (
      <div className="space-y-4">
        <div>
          <label htmlFor="libraryPlatform" className="block text-sm font-medium text-gray-700">
        ...
        {selectedLibPlatform?.needsLoginUrl && (
          <div>
            <label htmlFor="libLoginUrl" className="block text-sm font-medium text-gray-700">
        ...
        {selectedLibPlatform?.needsBaseUrl && (
          <div>
            <label htmlFor="libBaseUrl" className="block text-sm font-medium text-gray-700">
        ...
        <div>
          ...
          <div className="space-y-2">
            <label className="flex items-center">
              <input
                ...
                onChange={(e) => setLibLoginMethod(e.target.value as 'credentials' | 'cookies')}
                className="mr-2"
              />
          ...
          {libLoginMethod === 'cookies' && (
            <p className="text-xs text-blue-600 mt-1">{tPlatform('cookiesFileDescription')}</p>
          )}
        </div>
        ...
        {libLoginMethod === 'credentials' ? (
          <>
            <div>
              <label htmlFor="libUsername" className="block text-sm font-medium text-gray-700">
                {t('usernameOrEmail')}
              </label>
              <input
                type="text"
            ...
            <div>
              <label htmlFor="libPassword" className="block text-sm font-medium text-gray-700">
                {t('password')}
              </label>
              <input
                type="password"
          ...
          <div>
            <label htmlFor="libCookiesFile" className="block text-sm font-medium text-gray-700">
            ...
            />
            {libCookiesFile && (
              <p className="text-xs text-gray-500 mt-1">Selected: {libCookiesFile.name}</p>
            )}
          </div>
        )}
        ...
        <p className="text-xs text-gray-500">{tPlatform('seleniumAuthDescription')}</p>
        ...
        <div className="flex space-x-3">
          <AppButton 
            ...
            disabled={
              ...
              (libLoginMethod === 'credentials' ? (!libUsername || !libPassword) : !libCookiesFile)
            }
          >
  ...
  const LibraryBooksContent = () => {
    const loggedInPlatform = libraryPlatforms.find(p => p.id === loggedInLibraryPlatform);
        ...
        <div className="space-y-4">
          {books.length === 0 ? (
            <div className="text-center text-gray-400 py-8">
              {bookSearchQuery ? t('noBooks') : `${t('noBooksAvailable')} ${loggedInPlatform?.name || 'this platform'}`}
            </div>
          ) : (
            <>
              <p className="text-sm text-gray-600">
                {t('showingBooks', { count: books.length, plural: books.length !== 1 ? 's' : '' })} {loggedInPlatform?.name}
              </p>
              {books.map((book) => (
                <BookCard 
                  key={book.id} 
                  ...
                  downloadText={downloadingBooks.has(book.id) ? t('downloading') : t('downloadBook')}
  ...
  const LibrariesContent = () => {
    ...
    return <Tabs tabs={libraryTabs} activeTab={libraryTab} onTabChange={setLibraryTab} />;
  };

  const mainTabs = [
  { id: 'courses', label: t('coursesTab'), content: <Tabs tabs={platformTabs} activeTab={platformTab} onTabChange={setPlatformTab} /> },
  { id: 'libraries', label: t('librariesTab'), content: <LibrariesContent /> },
  { id: 'schedule-tasks', label: t('scheduleTasksTab'), content: <div className="text-center text-gray-400 py-8">{t('scheduleTasksComingSoon')}</div> },
  ];
