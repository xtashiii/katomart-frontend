...
interface DownloadSettingsProps {
  onStart: (config: { downloadPath: string; createSubfolders: boolean }) => void;
  config: { downloadPath: string; createSubfolders: boolean };
  setConfig: React.Dispatch<React.SetStateAction<{ downloadPath: string; createSubfolders: boolean }>>;
  disabled?: boolean;
}

export default function DownloadSettings({ onStart, config, setConfig, disabled }: DownloadSettingsProps) {
    ...
    <div className="space-y-4">
      <div>
        ...
        <input
          ...
          onChange={(e) => setConfig(c => ({ ...c, downloadPath: e.target.value }))}
      ...
      <div className="flex items-center">
        <input
          ...
          onChange={(e) => setConfig(c => ({ ...c, createSubfolders: e.target.checked }))}
          className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
          disabled={disabled}
        />
        <label htmlFor="createSubfolders" className="ml-2 block text-sm text-gray-900">
