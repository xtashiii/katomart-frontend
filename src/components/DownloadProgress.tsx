...
export default function DownloadProgress({
  ...
  const getModuleProgress = (module: Module) => {
    const selectedLessons = module.lessons.filter((lesson) =>
      isSelected(lesson.id)
    );
    if (!selectedLessons.length) return 0;
    return (
      selectedLessons.reduce((sum, l) => sum + getLessonProgress(l.id), 0) /
  ...
  return (
    <div>
      ...
      {courses
        .filter((course) => hasSelectedContent(course))
        .map((course) => (
          <div key={course.id} className="mb-6 border rounded-lg bg-gray-50">
            <div className="px-4 pt-4 pb-2 font-bold text-primary-dark flex items-center">
              ...
              <span className="ml-auto text-xs text-gray-500">
                {getCourseProgress(course).toFixed(0)}%
              </span>
            </div>
            ...
            <div className="ml-6">
              {course.modules
                .filter((module) => hasSelectedLessons(module))
                .map((module) => (
                  <div key={module.id} className="mb-4">
                    <div className="font-semibold text-gray-700 flex items-center">
                      <span>{module.name}</span>
                      <span className="ml-auto text-xs text-gray-500">
                        {getModuleProgress(module).toFixed(0)}%
                      </span>
                    </div>
                    ...
                    <div className="ml-6">
                      {module.lessons
                        .filter((lesson) => isSelected(lesson.id))
                        .map((lesson) => (
                          <div
                            key={lesson.id}
                          ...
                          >
                            ...
                            <div className="w-32 bg-gray-200 rounded-full h-1 mx-2">
                              <div
                                ...
                                style={{
                                  width: `${getLessonProgress(lesson.id)}%`,
                                }}
                              ></div>
                            </div>
                            <span className="ml-auto text-xs text-gray-500">
                              {getLessonProgress(lesson.id).toFixed(0)}%
                            </span>
                          </div>
                        ))}
                    </div>
