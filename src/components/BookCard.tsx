...
export default function BookCard({ book, onDownload, downloadText, isDownloading = false }: BookCardProps) {
    ...
    <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 p-4 border border-gray-200">
      <div className="flex items-start space-x-4">
        <div className="w-16 h-20 bg-gray-200 rounded-md flex items-center justify-center text-gray-500 text-xs flex-shrink-0">
          {book.cover ? (
            <Image src={book.cover} alt={book.title} width={64} height={80} className="w-full h-full object-cover rounded-md" />
          ) : (
            t('bookCard.noCover')
          )}
        </div>
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-gray-600 mb-1">{t('bookCard.by')} {book.author}</p>
          ...
          {book.description && (
            <p className="text-sm text-gray-500 line-clamp-2 mb-3" title={book.description}>
              {book.description}
            </p>
          )}
          <button
