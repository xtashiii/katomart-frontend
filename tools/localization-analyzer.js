...
class LocalizationAnalyzer {
  ...
  analyze() {
    ...
    for (const comp of comparisons) {
      console.log(
        `🔄 COMPARISON: ${comp.lang1.toUpperCase()} vs ${comp.lang2.toUpperCase()}`
      );

      if (comp.onlyIn1.length > 0) {
        console.log(
          `❌ Keys only in ${comp.lang1.toUpperCase()} (${comp.onlyIn1.length}):`
        );
        if (this.options.verbose) {
          ...
          console.log(
            `  ${comp.onlyIn1.slice(0, 5).join(', ')}${comp.onlyIn1.length > 5 ? '...' : ''}`
          );
        }
      ...
      if (comp.onlyIn2.length > 0) {
        console.log(
          `❌ Keys only in ${comp.lang2.toUpperCase()} (${comp.onlyIn2.length}):`
        );
        if (this.options.verbose) {
          ...
          console.log(
            `  ${comp.onlyIn2.slice(0, 5).join(', ')}${comp.onlyIn2.length > 5 ? '...' : ''}`
          );
        }
        hasInconsistencies = true;
      }
    ...
    console.log(
      `Structure consistency: ${hasInconsistencies ? '❌ INCONSISTENT' : '✅ CONSISTENT'}`
    );
